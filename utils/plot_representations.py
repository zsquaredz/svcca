...
def plot_embedding_layer_representation_1():
    # random.seed(30)
    ...
    for i in range(4):
        ...
        random.seed(30)
        indices_exp = (random.sample(range(0,general_data.shape[0]), k=3000))
        indices_con = (random.sample(range(0,control_data.shape[0]), k=3000))
        data = {}
...
def plot_embedding_layer_representation_model():
    # random.seed(30)
    # indices = (random.sample(range(0,117499),k=2500)) # books: 430923 clothing: 117499

    plt.figure(dpi=600)
    exp_names1 = ['25_model_100_data','50_model_100_data','75_model_100_data','100_model_100_data']
    exp_names2 = ['new_control_25_model_100_data','new_control_50_model_100_data','new_control_75_model_100_data','new_control_100_model_100_data']
    epoch1s = ['501','501','201','131']
    epoch2s = ['501','501','251','181'] # books 
    for i in range(4):
        EXP_NAME1=exp_names1[i]
        MODEL_CAT1='top5'
        epoch1=epoch1s[i]
...
def plot_embedding_layer_representation_with_mask2():
    ...
    for i in range(4):
        ...
        # for label, marker, color in zip(['control', 'control-specific'], [ (5,2), (5,2)], ['red', 'red']):
            ...
            if label == 'experimental':
                texts=[ax.text(X_temp[idx,0], X_temp[idx,1], 'unicode: U+25B2', fontsize=14, color='black') for idx in [100, 1, 202, 3218, 1153, 354, 1136, 921, 4384, 1898]]
                ...
                texts=[ax.text(X_temp[idx,0], X_temp[idx,1], 'unicode: U+25B5', fontsize=14, color='black') for idx in [561, 343, 0, 134, 101, 348, 885, 157, 292, 442]]
                ...
                texts=[ax.text(X_temp[idx,0], X_temp[idx,1], 'unicode: U+25B2', fontsize=14, color='black') for idx in indices_gen1]
            elif label == 'control-specific':
                texts=[ax.text(X_temp[idx,0], X_temp[idx,1], 'unicode: U+25B5', fontsize=14, color='black') for idx in indices_spe1]
            adjust_text(texts)
        if i==33:
            legend = ax.legend()
            h, l = ax.get_legend_handles_labels()
...
def plot_final_layer_representation_1():
    # random.seed(30)
    # indices = (random.sample(range(0,117499),k=2500)) # books: 430923 clothing: 117499

    plt.figure(dpi=600)
    exp_names1 = ['25_model_10_data','25_model_100_data','100_model_10_data','100_model_100_data']
    exp_names2 = ['new_control_25_model_10_data','new_control_25_model_100_data','new_control_100_model_10_data','new_control_100_model_100_data']
    epoch1s = ['501','501','251','131']
    epoch2s = ['501','501','101','181'] # books
    # epoch2s = ['501','501','501','501','501'] # clothing
    for i in range(4):
...
def plot_final_layer_representation_model():
    # random.seed(30)
    # indices = (random.sample(range(0,117499),k=2500)) # books: 430923 clothing: 117499
...
def plot_final_layer_representation_with_mask_model():
    # random.seed(30)
    # indices = (random.sample(range(0,117499),k=2500)) # books: 430923 clothing: 117499
